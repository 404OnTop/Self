...
#This took lots of hours to work on. Signing up to github takes about 3 minutes max, starring my post  - https://github.com/404 - takes 5 seconds. If you wouldn't mind pressing the star I'd really appreciate it!
#Have fun :)
...
def screen():
    ...
    while True:
        ...
        os.system(f"title 404 Selfbot - Running on {scalic.user.name} - Enjoy")
        logo = f"""



				        {Fore.WHITE}Connected To : {Fore.WHITE}[{Fore.MAGENTA}{scalic.user.name}#{scalic.user.discriminator}{Fore.WHITE}]




				        Messages Sent : {Fore.WHITE}[{Fore.MAGENTA}{messagessent}{Fore.WHITE}]




				        Commands Used : {Fore.WHITE}[{Fore.MAGENTA}{Fore.WHITE}]




				        Total Guilds : {Fore.WHITE}[{Fore.MAGENTA}{len(scalic.guilds)}{Fore.WHITE}]




				        Time Elapsed : {Fore.WHITE}[{Fore.MAGENTA}{'%d:%d:%d' %(hours,minutes,seconds)}{Fore.WHITE}]

                        .gg/GKDDJ8GQK9 | MOTD: Whole lotta stick play nigga
...
async def suggest(ctx,*,message="Supply something to suggest lol!"):
    ...
    try:
        embed=discord.Embed(title=f"404 Selfbot - poll", description=f"Poll message : `{message}`", color=randcolor)
...
async def ip(ctx, *, iparg="1.1.1.1"):
    ...
    if personalip in desc:
        ...
        embed=discord.Embed(title=f"404 Selfbot - ip locator", description="Your ip was detected in this data - to double check that your ok with showing the data\npress the reaction button below!", color=randcolor)
        ...
        while reactionstuffyes:
            try:
                ...
                embed=discord.Embed(title=f"404 Selfbot - ip locator", description=desc, color=randcolor)
                ...
                embed=discord.Embed(title=f"404 Selfbot - Ip locator", description="You took too long to react - try running this command again or sum", color=randcolor)
                await message.edit(embed=embed)
...
async def tokeninfo(ctx, bokenxd):
    ...
    # user info
        ...
        embed=discord.Embed(title=f"404 Selfbot - Token info", description=f"Site responded with status code : `{data.status_code}`\nMessage : `{data.text}`", color=randcolor)
        await ctx.send(embed=embed)
...
async def ping(ctx,afkstatus=None):
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title=f"404 Selfbot - Ping", description=f'🏓 pong | {round(scalic.latency * 1000)}ms', color=randcolor)
    await ctx.send(embed=embed)
...
async def afk(ctx,afkstatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Afk auto messager", description=f"Afk messages are now : `{isafk}`\nTry {prefix.strip()}afkmessage [message]", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
async def afkmsg(ctx,*,afkstatus="I'm afk , wait for me to reply"):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Afk auto messager", description=f"Afk messages are : `{isafk}`\nAfk message is : `{messagetosendwhenafk}`", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
async def uptime(ctx):
    ...
    seconds = timedata - 60*minutes
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Uptime", description=f"Scalic has been running for `{'%d:%d:%d' %(hours,minutes,seconds)}`", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
async def gamecycle(ctx):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Game Cycle", description=f"Cycling these statuses:\n`{statusdata}`\nDelay:\n`{delaybetweencycle}`", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
    while True:
...
async def biocycle(ctx):
    ...
    global delaybetweencycle
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Bio Cycle", description=f"Cycling these statuses:\n`{statusdata}`\nDelay:\n`{delaybetweencycle}`", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
async def watchcycle(ctx):
    ...
    global delaybetweencycle
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Watch Cycle", description=f"Cycling these statuses:\n`{statusdata}`\nDelay:\n`{delaybetweencycle}`", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
    while True:
...
async def nicknamecycle(ctx,statustocycle=None):
    ...
    if statustocycle == None or statustocycle.lower() == "yes" or statustocycle.lower() == "on":
        ...
        embed=discord.Embed(title="404 Selfbot - Nickname Cycle", description=f"Cycle status : `{nicknamestuffyes}`\n{prefix.strip()}nicknamecycle **to start the cycle**", color=randcolor)
        await ctx.message.edit(content="",embed=embed)
...
async def nicknamecycledelay(ctx,newdelay="5"):
    ...
    embed=discord.Embed(title="404 Selfbot - Nickname Delay", description=f"Delay is now : `{delaybetweennicknamecycle} seconds`\n{prefix.strip()}nicknamecycle", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
async def cycledelay(ctx,newdelay="5"):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Cycle Delay", description=f"Delay is now : `{delaybetweencycle} seconds", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
async def listencycle(ctx):
    ...
    global delaybetweencycle
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Listen Cycle", description=f"Cycling these statuses:\n`{statusdata}`\nDelay:\n`{delaybetweencycle}`", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
    while True:
...
async def streamcycle(ctx):
    ...
    global streamurl
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Stream Cycle", description=f"Cycling these statuses:\n`{statusdata}`\nDelay:\n`{delaybetweencycle}`", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
    while True:
...
async def listen(ctx, *, listenstatus="I didn't know how to specify a listen status lol"):
    randcolor = random.randint(0x000000, 0xFFFFFF)
    await scalic.change_presence(activity=discord.Activity(type=discord.ActivityType.listening, name=listenstatus))
    embed=discord.Embed(title="404 Selfbot - Listen Status", description=f"Status is now : `Listening to {listenstatus}`", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
async def game(ctx, *, gamestatus="I didn't know how to specify a game status lol"):
    ...
    embed=discord.Embed(title="404 Selfbot - Game Status", description=f"Status is now : `Playing {gamestatus}`", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
async def watch(ctx, *, watchstatus ="I didn't know how to specify a watch status lol"):
    randcolor = random.randint(0x000000, 0xFFFFFF)
    await scalic.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name=watchstatus))    
    embed=discord.Embed(title="404 Selfbot - Watch Status", description=f"Status is now : `Watching {watchstatus}`", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
async def stream(ctx, *, streamstatus): 
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    await scalic.change_presence(activity=stream)  
    embed=discord.Embed(title="404 Selfbot - Stream Status", description=f"Status is now : `Streaming {streamstatus}`", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
async def twitchurl(ctx, streamurltouse): 
    ...
    embed=discord.Embed(title="404 Selfbot - Stream Url Changed", description=f"Streamurl is now : `{streamurl}`\nTry {prefix.strip()}stream or {prefix.strip()}streamcycle", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
async def bitcoin(ctx):
    ...
    dolor = j['USD']
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Bitcoin command", description=f"Bitcoin value : `${dolor}`", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
async def invite(ctx): 
    ...
    embed=discord.Embed(title="404 Selfbot -Invite Creation", description=f"Invite Made - https://discord.gg/{str(inv.json()['code'])}", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
async def timetodeleteafter(ctx,deletetime):
    ...
    embed=discord.Embed(title="404 Selfbot - Delete after time Edited", description=f"Delete after time is now : `{deletetime} seconds`", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
async def deletemessagesafter(ctx,delstatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Delete after", description=f"Deleting messages after they've been sent : `{deleteafter}`\nTime between them being sent and deletion : `{deleteaftertime}`", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
def ssspam(webhook):
    ...
    while spammingdawebhookeroos:
        ...
        data = {
          ...
          "embeds": [
            {
              ...
              "fields": [
                {
                  "name": "404 Nuker",
                  "value": "404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker "
                },
                {
                  "name": "Nuked by 404 Nuked by 404",
                  "value": "404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker 404 Nuker"
                },
                {
                  "name": "404 Nuker 404 Nuker 404 Nuker",
              ...
              "footer": {
                "text": "404 Nuker Scalic Nuker Scalic Nuker Scalic Nuker Scalic Nuker Scalic Nuker Scalic Nuker Scalic Nuker Scalic Nuker Scalic Nuker Scalic Nuker Scalic Nuker Scalic Nuker Scalic Nuker Scalic Nuker Scalic Nuker Scalic Nuker Scalic Nuker Scalic Nuker",
                "icon_url": "https://avatars.githubusercontent.com/u/78554732?s=200&v=4"
              },
              "image": {
                "url": "https://avatars.githubusercontent.com/u/78554732?s=200&v=4"
          ...
          "username": "404 nuker scalic nuker - an advanced selfbot made open source on github! @@@@",
...
async def hypesquad(ctx,squad=None):
    ...
    if squad == None:
        ...
        if data.status_code == 204:
            embed=discord.Embed(title="404 Selfbot - Hypesquad Changer", description=f"Hypesquad changed successfully", color=randcolor)
            await ctx.message.edit(content="",embed=embed)
        else:
            embed=discord.Embed(title="404 Selfbot - Hypesquad Changer", description=f"Error - Site responded with status code : `{data.status_code}`\nMessage : `{data.text}`", color=randcolor)
            await ctx.message.edit(content="",embed=embed)
...
async def fucktoken(ctx,tokentofrick=None,*,nukemsg=f"This account was nuked with the help of scalic! - GG"):
    ...
    if tokentofrick == None:
        embed=discord.Embed(title="404 Selfbot - Token Fuck", description=f"Supply a token - {prefix.strip()}tokenfuck [token-here] [message-to-nuke-with]", color=randcolor)
        await ctx.message.edit(content="",embed=embed)


    elif ctx.guild == None:
        embed=discord.Embed(title="404 Selfbot - Token Fuck", description=f"Try do this command in a private server - it has problems in dms!", color=randcolor)
        await ctx.message.edit(content="",embed=embed)

    else:
        embed=discord.Embed(title="404 Selfbot - Token Fuck", description=f"If your sure you want to token fuck the account, react with the emoji below.\nThis process will dm all open dms , then close them, leave all servers, and make a ton of servers", color=randcolor)
        message = await ctx.send(embed=embed)        
        await message.add_reaction('✅')
        randcolor = random.randint(0x000000, 0xFFFFFF)
        reactionstuffyes = True
        ...
        while reactionstuffyes:
            try:
                reaction, user = await scalic.wait_for('reaction_remove', timeout=10, check=requirements)
                embed=discord.Embed(title="404 Selfbot - Token Fuck", description=f"You reacted, the process has started!\nValidating token...", color=randcolor)
                await message.edit(embed=embed)
                await message.clear_reactions()
                reactionstuffyes = False
                ...
                if tokendata.status_code != 200:
                    embed=discord.Embed(title="404 Selfbot - Token Fuck", description=f"Error - are you sure that token is correct?\nDiscord responded with : `{tokendata.text}`", color=randcolor)
                    await message.edit(embed=embed)           
                else:
                    embed=discord.Embed(title="404 Selfbot - Token Fuck", description=f"Token valid - continuing the process", color=randcolor)
                    await message.edit(embed=embed)      
                    ...
                    randcolor = random.randint(0x000000, 0xFFFFFF)
                    embed=discord.Embed(title="404 Selfbot - Token Fuck", description=f"Messaged {usersmessaged} people saying : `{nukemsg}` and cleared the conversation after", color=randcolor)
                    await message.edit(embed=embed)              
                    ...
                    randcolor = random.randint(0x000000, 0xFFFFFF)
                    embed=discord.Embed(title="404 Selfbot - Token Fuck", description=f"Left `{serversleft}` servers", color=randcolor)
                    await message.edit(embed=embed)  
                    ...
                    randcolor = random.randint(0x000000, 0xFFFFFF)
                    embed=discord.Embed(title="404 Selfbot - Token Fuck", description=f"Deleted `{serversdeleted}` servers", color=randcolor)
                    try:
                        await message.edit(embed=embed)  
                    except:
                        pass
                    ...
                    randcolor = random.randint(0x000000, 0xFFFFFF)
                    embed=discord.Embed(title="404 Selfbot - Token Fuck", description=f"Made {serversmade} servers", color=randcolor)
                    try:
                        await message.edit(embed=embed)  
                    except:
                        pass
                ...
                embed=discord.Embed(title="404 Selfbot - Token Fuck", description=f"You took too long - Run this command again if you wish to token fuck an account", color=randcolor)
                await message.edit(embed=embed)
                await message.clear_reactions()
                reactionstuffyes= False
...
async def tokendisable(ctx,tokentofrick=None):
    ...
    await ctx.message.delete()
    if tokentofrick == None:
        embed=discord.Embed(title="404 Selfbot - Token Disabler", description=f"Supply a token - {prefix.strip()}tokendisable [token-here] ", color=randcolor)
        await ctx.message.edit(content="",embed=embed)


    elif ctx.guild == None:
        embed=discord.Embed(title="404 Selfbot - Token Disabler", description=f"Try do this command in a private server - it has problems in dms (working on fixing)!", color=randcolor)
        await ctx.message.edit(content="",embed=embed)

    else:
        embed=discord.Embed(title="404 Selfbot - Token Disabler", description=f"If your sure you want to disable that token, react.\nDisabling means that the account will be DISABLED - the chances of recovering it are pratically impossible.", color=randcolor)
        message = await ctx.send(embed=embed)        
        await message.add_reaction('✅')
        randcolor = random.randint(0x000000, 0xFFFFFF)
        reactionstuffyes = True
        ...
        while reactionstuffyes:
            try:
                reaction, user = await scalic.wait_for('reaction_remove', timeout=10, check=requirements)
                embed=discord.Embed(title="404 Selfbot - Token Disabler", description=f"You reacted, the process has started!\nDisabling token", color=randcolor)
                await message.edit(embed=embed)
                await message.clear_reactions()
                reactionstuffyes = False
                ...
                randcolor = random.randint(0x000000, 0xFFFFFF)
                if tokendata.status_code != 200:
                    embed=discord.Embed(title="404 Selfbot - Token Disabler", description=f"Token disabled - discord responds with : `{tokendata.text}`", color=randcolor)
                    await message.edit(embed=embed)           
                else:
                    embed=discord.Embed(title="404 Selfbot - Token Disabler", description=f"Token valid - strange - try again maybe?", color=randcolor)
                    await message.edit(embed=embed)      

            except asyncio.TimeoutError:
                embed=discord.Embed(title="404 Selfbot - Token Disabler", description=f"You took too long - Run this command again if you wish to DISABLE an account", color=randcolor)
                await message.edit(embed=embed)
                await message.clear_reactions()
                reactionstuffyes= False
...
async def editsnipe(ctx):
    ...
    if str(ctx.channel.id) in editedmessages:
        ...
        embed=discord.Embed(title="404 Selfbot - Edit snipe", description=f"Message sniped:\n{msgsniped}", color=randcolor)
        await ctx.message.edit(content="",embed=embed)

    else:
        embed=discord.Embed(title="404 Selfbot - Edit snipe", description=f"No edited messages to snipe", color=randcolor)
        await ctx.message.edit(content="",embed=embed)
...
async def unban(ctx, userid="Nonexd"):
    ...
    if len(str(userid)) != 18:
        embed=discord.Embed(title="404 Selfbot - Unban Command", description=f"\nCommand usage : {prefix.strip()}unban [id]", color=randcolor)
        await ctx.message.edit(content="",embed=embed)   
    else:
...
async def banid(ctx, userid="Nonexd",reason="None specified"):
    randcolor = random.randint(0x000000, 0xFFFFFF)
    if len(str(userid)) != 18:
        embed=discord.Embed(title="404 Selfbot - Ban ID Command", description=f"\nCommand usage : {prefix.strip()}banid [id]", color=randcolor)
        await ctx.message.edit(content="",embed=embed)   
    else:
        ...
        try:
            ...
            embed=discord.Embed(title="404 Selfbot - Ban Command", description=f"\nBanned {userid}", color=randcolor)
            await ctx.message.edit(content="",embed=embed)   
...
async def kick(ctx, member : discord.Member,reason="None specified"):
    ...
    if member.id == ctx.author.id:
        embed=discord.Embed(title="404 Selfbot - Kick Command", description=f"\nTry {prefix.strip()}leave [server-id] instead!", color=randcolor)
        await ctx.message.edit(content="",embed=embed) 
    else:
...
async def downloadguildemojis(ctx, guildid=None):
    if guildid == None:
        ...
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title=f"404 Selfbot - Emoji stealing from {guildtostealfrom.name}", description=f"Details:\nSuccessful emoji steals : {emojisuccess}\nErrors with emoji steals : {emojierror}", color=randcolor)
        await ctx.message.edit(content="",embed=embed)
...
async def stealguildemoji(ctx, guildid=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title=f"404 Selfbot - Finished emoji stealing from {guildtostealfrom.name}", description=f"Details:\nSuccessful emoji steals : {emojisuccess}\nErrors with emoji steals : {emojierror}\n{emojilist}", color=randcolor)
    await ctx.message.edit(embed=embed)
...
async def embedspam(ctx, count=5, *, desc="I forgot to specify the embed content"):
    ...
    if int(count) > 25:
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="404 Selfbot - embed spam", description=f"{count} embeds is too much - do 25 or less", color=randcolor)
        await ctx.send(embed=embed)
    else:
        for i in range(int(count)):
            randcolor = random.randint(0x000000, 0xFFFFFF)
            embed=discord.Embed(title="404 Selfbot - embed spam", description=desc, color=randcolor)
            await ctx.send(embed=embed)
...
async def spam(ctx, count=5, *, desc="I forgot to specify the content to spam"):
    ...
    if int(count) > 25:
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="404 Selfbot - spam", description=f"{count} messages is too much - do 25 or less", color=randcolor)
        await ctx.send(embed=embed)
    else:
        for i in range(int(count)):
            await ctx.send(desc)
...
async def spampin(ctx, count=None):
    if count == None:
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="404 Selfbot - Pin spam", description=f"You didn't specify the amount of pins to do.\n{prefix.strip()}spampin [amount]", color=randcolor)
...
async def logout(ctx):
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Logging out", description=f"Logging out!", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
    await scalic.logout()
...
async def clear(ctx, amount):
    ...
    await ctx.message.delete()
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Message purger", description=f"Deleting messages", color=randcolor)
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Message purger", description=f"Finished.\nDeleted {deleted} messages", color=randcolor)
...
async def spamreact(ctx, count=None, reaction=None):
    ...
    if count == None or reaction == None:
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="404 Selfbot - React spam", description=f"You didn't specify the amount of messages to react to or the reaction to use.\n{prefix.strip()}spamreact [amount]", color=randcolor)
...
async def spamedit(ctx, count=None,*, mesg=None):
    ...
    if count == None or mesg == None:
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="404 Selfbot - Message edit spam", description=f"You didn't specify the amount of messages to edit or the content to edit the messages to.\n{prefix.strip()}spamedit [amount] [message]", color=randcolor)

        await ctx.send(embed=embed)
    else:
        edited = 0
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="404 Selfbot - Message edit spam", description=f"Editing messages", color=randcolor)
        msg= await ctx.send(embed=embed)
        ...
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="404 Selfbot - Message edit spam", description=f"Finished.\nEdited {edited} messages", color=randcolor)
        await msg.edit(embed=embed,delete_after=3)
...
async def tokenhalf(ctx, member: discord.Member):#
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Message edit spam", description=f"{member.name}'s Token Begins With : \n`{final[1:]}`", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
async def logdeletedmessages(ctx,deletestatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Deleted message logger", description=f"Deleted message logger is now : `{deletedmessagelogger}`", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
async def logediteddmessages(ctx,editstatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Edited message logger", description=f"Edited message logger is now : `{editedmessagelogger}`", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
async def edit(ctx,editstatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Edit mode", description=f"Edit mode is now : `{statusofediting}`", color=randcolor)
    await ctx.message.edit(content="",embed=embed)#
...
async def talkcute(ctx,talkcuteyems=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Cute talk", description=f"Edit mode is now : `{talkingcuteyems}`", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
async def nitrosnipe(ctx,snipestatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Nitro Sniper", description=f"Nitro sniper is now : `{nitrosniping}`", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
async def privnotesnipe(ctx,snipestatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Privnote Sniper", description=f"Privnote sniper is now : `{privnotesniping}`", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
async def notifications(ctx,notifstatushm=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Notifications", description=f"Notifications are now : `{notifsyems}`", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
async def giveawaysnipe(ctx,snipestatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Giveaway Sniper", description=f"Giveaway sniper is now : `{giveawaysniping}`", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
async def backup(ctx,snipestatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Backup", description=f"Backing up friends...", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Backup", description=f"Saved `{friendssaved}` friends", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Backup", description=f"Saved `{friendssaved}` friends and `{serverssaved}` servers", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
    ...
    f.close()
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Backup", description=f"Saved `{friendssaved}` friends , `{serverssaved}` servers and `{groupssaved}` group chats.", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
async def pastebinsnipe(ctx,snipestatus=None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Pastebin Sniper", description=f"Pastebin sniper is now : `{pastebinsniping}`", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
async def idtoname(ctx, personsid: int):
    if len(str(personsid)) != 18:
        ...
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="404 Selfbot - Id to username", description=f"ID [{str(personsid)}]  = `{user.name}#{user.discriminator}`", color=randcolor)
        await ctx.message.edit(content="",embed=embed)
...
async def hastebin(ctx, *,paste=f"Format is {prefix.strip()}hastebin [text]"):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Hastebin Command", description=f"Here's your paste!\n{textlink}{endoflink}", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
async def nuke(ctx):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Channel Nuked", description=f"<#{new_chan.id}> - {new_chan.name} has been nuked", color=randcolor)
...
async def tinyurl(ctx,link=None):
    ...
    await ctx.message.delete()
    if link == None:
        embed=discord.Embed(title=f"404 Selfbot - Url shortener", description=f"You didn't supply a link - {prefix.strip()}tinyurl [link-here]", color=randcolor)
        message = await ctx.send(embed=embed)
    else:
        embed=discord.Embed(title=f"404 Selfbot - Shortening link", description=f"beep bop beep", color=randcolor)
        message = await ctx.send(embed=embed)

        randcolor = random.randint(0x000000, 0xFFFFFF)
        shortened = requests.get(f"https://tinyurl.com/api-create.php?url={link}").text
        embed=discord.Embed(title=f"404 Selfbot - Url shortener", description=f"Link shortened : \n{shortened}", color=randcolor)
        message = await ctx.send(embed=embed)
...
async def join(ctx,link=None):#
    await ctx.message.delete()
    if link == None:
        embed=discord.Embed(title=f"404 Selfbot - Server joiner", description=f"You didn't supply an invite - {prefix.strip()}join discord.gg/server", color=randcolor)
        await ctx.send(embed=embed)
    else:
        headers = {'Authorization': token.strip(), 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/81.0.4044.138 Safari/537.36', 'Accept': '*/*',}
        ...
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title=f"404 Selfbot - Server joiner", description=f"Joining server : `discord.gg/{link}`", color=randcolor)
        message = await ctx.send(embed=embed)
        ...
        if joint == 200:
            ...
            embed=discord.Embed(title=f"404 Selfbot - Server joiner", description=f"Successfully joined : `discord.gg/{link}`", color=randcolor)
            

        else:
            embed=discord.Embed(title=f"404 Selfbot - Server joiner", description=f"Error Joining : `discord.gg/{link}`", color=randcolor)
        await message.edit(embed=embed)
...
async def leave(ctx,servid=None):#
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    if servid == None:
            embed=discord.Embed(title=f"404 Selfbot - Server joiner", description=f"Supply a link - {prefix.strip()}leave [server-id] \n(fyi this server id is {ctx.guild.id})", color=randcolor) #yes - i could make it do it in the guild its done it but idk, feel like people might accidently not supply a userid or sum
            await ctx.send(embed=embed)
    else:
        headers = {'Authorization': token.strip(), 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/81.0.4044.138 Safari/537.36', 'Accept': '*/*',}

        embed=discord.Embed(title=f"404 Selfbot - Server Leaver", description=f"Leaving `{servid}`", color=randcolor)
        message = await ctx.send(embed=embed)
        leave = requests.delete(f"https://discord.com/api/v8/users/@me/guilds/{servid}", headers=headers)
        randcolor = random.randint(0x000000, 0xFFFFFF)
        if leave.status_code == 204:
        
            embed=discord.Embed(title=f"404 Selfbot - Server Leaver", description=f"Success | Left Server : `{servid}`", color=randcolor)
            await message.edit(embed=embed)


        else:
            embed=discord.Embed(title=f"404 Selfbot - Server Leaver", description=f"Error | Error leaving server : `{servid}`\nMessage : {leave.text}", color=randcolor)
            await message.edit(embed=embed)
...
async def serverinfo(ctx):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title=f"404 Selfbot - Gathering info on {ctx.guild.name}", description="...", color=randcolor)
    message = await ctx.send(embed=embed)
    await asyncio.sleep(1)
    try:
...
"""
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title=f"404 Selfbot - {ctx.guild.name}", description=info, color=randcolor)
    await message.edit(embed=embed)
...
async def userinfo(ctx, member: discord.Member = None):
    ...
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - User info", description=f"Getting user info on {member.name}", color=randcolor)
    message = await ctx.send(embed=embed)
    await asyncio.sleep(2)
...
"""  
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title=f"404 Selfbot - {member.name}#{member.discriminator}", description=info, color=randcolor)
    await message.edit(embed=embed)
...
async def dmclear(ctx):
    ...
    await ctx.message.delete()
    randcolor = random.randint(0x000000, 0xFFFFFF)
    embed=discord.Embed(title="404 Selfbot - Message Clearer", description=f"Clearing all messages with all users", color=randcolor)
    msg= await ctx.send(embed=embed)
    for channel in scalic.private_channels:
        ...
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="404 Selfbot - Message Clearer", description=f"Clearing all messages with all users\nUsers Done : {usersdone}\nTotal Messages Deleted : {totalmessage}", color=randcolor)
...
async def joke(ctx):
    ...
    embed=discord.Embed(title="404 Selfbot - Joke requested", description=f"{setup}\n||{delivery}||", color=randcolor)
    await ctx.message.edit(content="",embed=embed)
...
async def ud(ctx,*,wordtodefine=None):
    ...
    if wordtodefine == None:  
        embed=discord.Embed(title="404 Selfbot - Urban Dictionary Command", description=f"You didn't supply a word to define?", color=randcolor)
        await ctx.message.edit(content="",embed=embed)
    else:
        ...
        try:
            ...
            embed=discord.Embed(title="404 Selfbot - Urban Dictionary Command", description=f"Definition of `{wordtodefine}`\n{finaldef}\n\n", color=randcolor)
            await ctx.message.edit(content="",embed=embed)
        except:
            embed=discord.Embed(title="404 Selfbot - Urban Dictionary Command", description=f"Error when searching for the term : `{wordtodefine}`", color=randcolor)
            await ctx.message.edit(content="",embed=embed)           
...
async def download(ctx):
    ...
    await ctx.message.edit(content="https://github.com/404 404 selfbot - 404 selfbot was made as a fun project.\n404 is free and has creative commands - why wouldn't you try it out\nIt has a wide range of commands, uses fun apis and is simple to setup - no api keys needed",embed=embed)  

@scalic.command(aliases=["gender"])
async def name(ctx,*,namesupplied=None):
    randcolor = random.randint(0x000000, 0xFFFFFF)
    if namesupplied == None:
        embed=discord.Embed(title="404 Selfbot - Gender command", description=f"You didn't supply a name\n{prefix.strip()}name [name]", color=randcolor)
        await ctx.message.edit(content="",embed=embed)
    else:
        ...
        embed=discord.Embed(title="404 Selfbot - Gender command", description=f"Majority of people named {namesupplied} are {gen}", color=randcolor)
        await ctx.message.edit(content="",embed=embed)
...
async def webhookdelete(ctx,link=None):
    randcolor = random.randint(0x000000, 0xFFFFFF)
    if link == None:
        embed=discord.Embed(title="404 Selfbot - No webhook supplied", description=f"{prefix.strip()}webhookdelete [webhook]", color=randcolor)
        await ctx.message.edit(content="",embed=embed)
    else:
        embed=discord.Embed(title="404 Selfbot - Deleting webhook", description=f"Sending a delete request to\n{link}", color=randcolor)
        await ctx.message.edit(content="",embed=embed)
        ...
        if result.status_code == 204:
            embed=discord.Embed(title="404 Selfbot - Webhook Deleted", description=f"Yay", color=randcolor)
            await ctx.message.edit(embed=embed)
        else:
            embed=discord.Embed(title="404 Selfbot - Error Deleting Webhook", description=f"Delete request responded with status code : {result.status_code}\{result.text}", color=randcolor)
            await ctx.message.edit(embed=embed)
